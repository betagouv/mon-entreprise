...
describe('ValueInput', () => {
	it('should render an input', () => {
		expect(getInput(<ValueInput />)).to.have.length(1)
	})

	it('should accept both . and , as decimal separator', () => {
		let onChange = spy()
		const input = getInput(<ValueInput onChange={onChange} />)
	...
	it('should separate thousand groups', () => {
		const input1 = getInput(<ValueInput value={1000} language="fr" />)
		const input2 = getInput(<ValueInput value={1000} language="en" />)
		const input3 = getInput(<ValueInput value={1000.5} language="en" />)
		const input4 = getInput(<ValueInput value={1000000} language="en" />)
	...
	it('should handle decimal separator', () => {
		const input = getInput(<ValueInput value={0.5} language="fr" />)
	...
	it('should change the position of the currency symbol depending on the language', () => {
		const inputFr = shallow(<ValueInput unit="€" language="fr" />)
		expect(
			inputFr
				.children()
				.last()
		...
		const inputEn = shallow(<ValueInput unit="€" language="en" />)
		expect(
			inputEn
				.children()
				.first()
	...
	it('should debounce onChange call', () => {
		...
		let onChange = spy()
		const input = getInput(<ValueInput onChange={onChange} debounce={1000} />)
	...
	it('should initialize with value of the value prop', () => {
		const wrapper = shallow(<ValueInput value={1} />)
	...
	it('should not call onChange the value is the same as the current input value', () => {
		let onChange = spy()
		const wrapper = mount(<ValueInput value={2000} onChange={onChange} />)
