...
const ditAvoirPlusDe2MoisDeDéclaration = (
	...
	if (situation.plusDe2MoisDeDéclaration.value) {
		...
		return inéligible('declarations')
	}
}
...
const nombreDeMoisEmployeureuseSuffisant = (
	...
	const historique = construireHistorique(situation.salariees)
...
const construireHistorique = (
	...
	const declarations = pipe(salariees, R.values, A.flatten)

	return {
		mars: declarationsPourLeMois(declarations, 'mars'),
		avril: declarationsPourLeMois(declarations, 'avril'),
		mai: declarationsPourLeMois(declarations, 'mai'),
	}
}

const declarationsPourLeMois = (
	salariees: Array<Salariée>,
	mois: Mois
): Array<DéclarationDeGarde> =>
	pipe(
		salariees,
		A.map((salariee) => salariee[mois]),
		A.getSomes
	)
...
export const moyenneHeuresParTypologieDeGarde =
	...
	(
		declarationsDeGarde: Array<DéclarationDeGarde>
...
export const auMoinsUnEnfantGardéOuvrantDroitAuCMG = (
	...
	if (A.isNonEmptyArray(situation.salariees.GED)) {
