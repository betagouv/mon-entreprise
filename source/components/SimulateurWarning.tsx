...
export function SimulateurWarning({ children, name }) {
	let [folded, fold] = usePersistingState(
		'app::simulateurs:warning-folded:v1:' + name,
		false
	)
	return (
		<div
...
export default function CotisationSimulateurWarning({
	simulateur
}: SimulateurWarningProps) {
	return (
		<SimulateurWarning name={simulateur}>
			<ul>
				{simulateur == 'auto-entreprise' && (
					<li>
						<T k="simulateurs.warning.cfe">
							...
							varie fortement en fonction du chiffre d'affaire et de la
				...
				{simulateur == 'auto-entreprise' && (
					<li>
						<T k="simulateurs.warning.auto-entrepreneur">
							...
							leur chiffre d'affaires. Il faut donc retrancher au net tous les
							coûts liés à l'entreprise pour obtenir le revenu réellement perçu.
						</T>
					</li>
				)}
			</ul>
