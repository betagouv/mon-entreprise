...
describe('Simulateur auto-entrepreneur', function () {
	...
	it(
		'devrait conserver les proportions de chiffre d’affaires mixte lorsque le CA total est modifié',
		{
		...
		function () {
			...
			cy.get(inputSelector)
				.eq(1)
				.then(($input) => {
					cy.wrap($input).type('{selectall}2500')
					cy.wrap($input)
						.invoke('val')
						.then(($value) => expect($value).to.be.equal('2 500 €'))
				})
			cy.get(inputSelector)
				.eq(2)
				.then(($input) => {
					cy.wrap($input).type('{selectall}5000')
					cy.wrap($input)
						.invoke('val')
						.then(($value) => expect($value).to.be.equal('5 000 €'))
				})
			cy.get(inputSelector)
				.eq(3)
				.then(($input) => {
			...
			cy.get(inputSelector)
				.first()
				.invoke('val')
				.should(($value) => expect($value).to.be.equal('10 000 €'))
