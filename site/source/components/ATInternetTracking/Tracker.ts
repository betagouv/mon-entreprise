...
export interface ATTracker {
	...
	setProperties(
		propertiesObject: {
			...
			simulateur_nom: string
			simulateur_origine: string
			simulateur_origine_page: string
		},
		options: {
			persistent: true
		}
...
export function createTracker(siteId?: string, doNotTrack = false) {
	...
	class PianoTracker implements ATTracker {
		constructor(options: { language: 'fr' | 'en' }) {
			...
			let integratorUrl = urlParams.get('integratorUrl');
			let integratorUrlDecoded = decodeURIComponent(integratorUrl);
			console.log("integratorUrlDecoded=" + integratorUrlDecoded);
			let dnsArray = integratorUrlDecoded.match(/^https?:\/\/([^\/]+)/);
			let integratorUrlDns = dnsArray[1];
			console.log("integratorUrlDns=" + integratorUrlDns);
			window.pa.setProperties(
				{
					...
					simulateur_nom: document.location.pathname,
					simulateur_origine: integratorUrlDns,
					simulateur_origine_page: integratorUrlDecoded,
				},
				{ persistent: true }
			)
		}
