...
export let
		...
		mecanismNumericalLogic = (recurse, k,v) =>
			R.ifElse(
				R.is(String),
		...
		mecanismPercentage = (recurse,k,v) => {
			let reg = /^(\d+(\.\d+)?)\%$/
			if (R.test(reg)(v))
				return {
					category: 'percentage',
