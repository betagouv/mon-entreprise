...
import { SalariéeAMAFactory, SalariéeGEDFactory } from './salarieeFactory'
...
describe('CMG', () => {
	describe('éligibilité', () => {
		...
		it('n’est pas éligible si moins de 2 mois employeureuse', () => {
			...
			expect(résultat).to.be.deep.equal(E.left(['declarations']))
		})
		...
		it('n’est pas éligible si a saisi moins de 2 declarations', () => {
			const résultat = éligibilité({
				_tag: 'Situation',
				aPerçuCMG: O.some(true) as O.Some<boolean>,
