...
type Releases = typeof import('@/public/data/releases')
...
export default function Nouveautés() {
	...
	const slug = matchPath(encodeURI(absoluteSitePaths.nouveautes.date))?.params
		.date
	useHideNewsBanner()
	...
	const getPath = (index: number) =>
		`${absoluteSitePaths.nouveautes.index}/${slugify(data[index].name)}`
	...
	return (
		<>
			...
			<Meta
				...
				description={t(
					'news.description',
					'Nous améliorons le site en continu à partir de vos retours. Découvrez les dernières nouveautes'
				)}
			/>
			...
			<Container>
				...
				<Body>
					...
					. Découvrez les{' '}
					{selectedRelease === 0
						? 'dernières nouveautes'
						: `nouveautes ${determinant(releaseName)}${releaseName}`}
					&nbsp;:
				</Body>
