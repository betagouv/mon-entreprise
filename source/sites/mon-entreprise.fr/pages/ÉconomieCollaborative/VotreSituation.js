...
export default withSitePaths(function VotreSituation({ sitePaths }) {
	...
	const titre = t('économieCollaborative.obligations.titre', 'Que dois-je faire pour être en règle ?')
				...
				{déclarations.RÉGIME_GÉNÉRAL_DISPONIBLE.length > 0 && (
					<>
						...
						<T k="économieCollaborative.obligations.entreprise">
							...
							<p>
								Si vous possédez déjà une activité déclarée, vous pouvez ajouter ces revenus à ceux de l'entreprise. Il vous faudra seulement vérifier que son objet social est compatible avec les activités concernées (et le changer si besoin).
								Sinon, vous aurez à créer une nouvelle entreprise.
							</p>
							<Link
				...
				<ActivitéCard
					...
					label={
						régimeGénéralDisponibleSelector(state, title)
							? <T k="économieCollaborative.obligations.régimeGénéralDisponible">Régime général disponible</T>
							: null
					}
				/>
			))}
