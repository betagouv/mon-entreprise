...
async function main() {
  createDataDir()
  writeStatsInDataDir(await fetchStats())
}
...
async function fetchStats() {
  try {
    ...
    let result = Object.fromEntries(
      ...
      Object.entries(data).map(([key, value]) => [key, value.
        filter(function (page) {
          return ['/salarié', '/auto-entrepreneur', '/artiste-auteur', '/indépendant', '/comparaison-régimes-sociaux', '/assimilé-salarié'].includes(page.label) // '/salarie' pas pris en compte
        }).
        map(function (page) {
    ...
    result = d3.nest()
      .key(function (p) { return p.month; })
      .key(function (p) { return p.label; })
      .rollup(function (v) { return d3.sum(v, function (p) { return p.nb_visits; }) })
      .entries(result);
    return result
  } catch (e) {
    console.log('toto')
...
function writeStatsInDataDir(stats) {
