...
describe('CMG', () => {
	describe('calculeComplémentTransitoire', () => {
		it('calcule le complément transitoire pour la garde de Jules', () => {
					...
					GED: [
						{
							...
							avril: O.some(
								new DéclarationsDeGardeGEDFactory()
									.avecNbHeures(12)
									.avecRémunération(MR.eurosParMois(48))
									.avecCMG(MR.eurosParMois(200))
									.build()
							),
							mai: O.some(
								new DéclarationsDeGardeGEDFactory()
					...
					AMA: [
						{
							mars: O.some(
								new DéclarationsDeGardeAMAFactory(['Jules'])
									.avecNbHeures(100)
									.avecRémunération(MR.eurosParMois(300))
									.avecCMG(MR.eurosParMois(250))
									.build()
							),
							avril: O.some(
								new DéclarationsDeGardeAMAFactory(['Jules'])
			...
			expect(résultat).to.be.deep.equal(MR.eurosParMois(49.15))
		})
		...
		it('calcule un complément transitoire nul pour la garde de Rose', () => {
			...
			expect(résultat).to.be.deep.equal(MR.eurosParMois(0))
		})
	})
	...
	describe('moyenneCMGPerçus', () => {
		it('fait la moyenne de tous les CMG perçus pour la garde de Jules et Martin', () => {
			const résultat = moyenneCMGPerçus({
				...
				AMA: [
					...
					},
					{
						...
						avril: O.some(
							new DéclarationsDeGardeAMAFactory(['Jules'])
								.avecCMG(MR.eurosParMois(230))
								.build()
						),
						mai: O.some(
							new DéclarationsDeGardeAMAFactory(['Martin'])
								.avecCMG(MR.eurosParMois(150))
								.build()
						),
					},
					{
			...
			expect(résultat).to.be.deep.equal(MR.eurosParMois(553.33))
		})
		...
		it('fait la moyenne de tous les CMG perçus pour la garde de Jules', () => {
			...
			expect(résultat).to.be.deep.equal(MR.eurosParMois(220))
		})
	})
	...
	describe('moyenneCMGRLinéarisés', () => {
		it('calcule la moyenne des CMG-R linéarisés', () => {
			...
			expect(résultat).to.be.deep.equal(MR.eurosParMois(170.85))
		})
	})
	...
	describe('calculeCMGRLinéarisé', () => {
		it('calcule le CMG-R linéarisé pour 100h de garde AMA à 300€', () => {
			...
			expect(résultat).to.be.deep.equal(MR.eurosParMois(220.21))
		})
		...
		it('calcule le CMG-R linéarisé pour 12h de garde GED à 48€', () => {
			...
			expect(résultat).to.be.deep.equal(MR.eurosParMois(36.07))
		})
	})
	...
	describe('coûtMensuelDeLaGarde', () => {
		it('utilise le coût de la garde renseigné pour une garde AMA inférieure au plafond', () => {
			const résultat = coûtMensuelDeLaGarde(
				new DéclarationsDeGardeAMAFactory(['Rose'])
					.avecNbHeures(100)
					.avecRémunération(MR.eurosParMois(700))
					.build()
			)

			expect(résultat).to.be.deep.equal(MR.eurosParMois(700))
		})
		...
		it('utilise le taux horaire plafond pour une garde AMA supérieure au plafond', () => {
			const résultat = coûtMensuelDeLaGarde(
				new DéclarationsDeGardeAMAFactory(['Rose'])
					.avecNbHeures(100)
					.avecRémunération(MR.eurosParMois(900))
					.build()
			)

			expect(résultat).to.be.deep.equal(MR.eurosParMois(800))
		})
		...
		it('utilise le coût de la garde renseigné pour une garde GED inférieure au plafond', () => {
			const résultat = coûtMensuelDeLaGarde(
				new DéclarationsDeGardeGEDFactory()
					.avecNbHeures(100)
					.avecRémunération(MR.eurosParMois(400))
					.build()
			)

			expect(résultat).to.be.deep.equal(MR.eurosParMois(400))
		})
		...
		it('utilise le taux horaire plafond pour une garde GED supérieure au plafond', () => {
			const résultat = coûtMensuelDeLaGarde(
				new DéclarationsDeGardeGEDFactory()
					.avecNbHeures(100)
					.avecRémunération(MR.eurosParMois(1600))
					.build()
			)

			expect(résultat).to.be.deep.equal(MR.eurosParMois(1500))
		})
	})
})
