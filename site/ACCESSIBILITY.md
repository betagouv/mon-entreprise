...
-   Toujours associer un `label` à chaque `input` (un placeholder ne suffit pas).
-   Toujours mettre une liaison `for/id` entre un `input` et son `label`, même lorsqu'ils sont imbriqués.
-   Les groupes de boutons radio ou de checkbox doivent être contenus dans un `<fieldset>` avec une `<legend>`.
