...
var CaptureConsole = class _CaptureConsole {
  /**
   * @inheritDoc
   */
  static __initStatic() {
  ...
  setupOnce(_, getCurrentHub) {
    ...
    addInstrumentationHandler("console", ({ args, level }) => {
      ...
      if (!hub.getIntegration(_CaptureConsole)) {
        return;
      }
...
var Dedupe = class _Dedupe {
  ...
  setupOnce(addGlobalEventProcessor, getCurrentHub) {
    const eventProcessor = (currentEvent) => {
      ...
      const self2 = getCurrentHub().getIntegration(_Dedupe);
      if (self2) {
        try {
...
var ExtraErrorData = class _ExtraErrorData {
  /**
   * @inheritDoc
   */
  static __initStatic() {
  ...
  setupOnce(addGlobalEventProcessor, getCurrentHub) {
    addGlobalEventProcessor((event, hint) => {
      const self2 = getCurrentHub().getIntegration(_ExtraErrorData);
      if (!self2) {
        return event;
      }
...
var ReportingObserver = class _ReportingObserver {
  /**
   * @inheritDoc
   */
  static __initStatic() {
   ...
   */
  constructor(options = {}) {
    this.name = _ReportingObserver.id;
  ...
  handler(reports) {
    ...
    if (!hub || !hub.getIntegration(_ReportingObserver)) {
      return;
    }
...
var RewriteFrames = class _RewriteFrames {
  ...
  setupOnce(addGlobalEventProcessor, getCurrentHub) {
    addGlobalEventProcessor((event) => {
      const self2 = getCurrentHub().getIntegration(_RewriteFrames);
      if (self2) {
...
var SessionTiming = class _SessionTiming {
  /**
   * @inheritDoc
   */
  static __initStatic() {
  ...
  setupOnce(addGlobalEventProcessor, getCurrentHub) {
    addGlobalEventProcessor((event) => {
      const self2 = getCurrentHub().getIntegration(_SessionTiming);
      if (self2) {
        return self2.process(event);
      }
      return event;
...
var Transaction = class _Transaction {
  ...
  setupOnce(addGlobalEventProcessor, getCurrentHub) {
    addGlobalEventProcessor((event) => {
      const self2 = getCurrentHub().getIntegration(_Transaction);
      if (self2) {
        return self2.process(event);
      }
      return event;
...
var ContextLines = class _ContextLines {
  ...
  addSourceContext(event) {
    ...
    const htmlFilename = WINDOW3.location && stripUrlQueryAndFragment(WINDOW3.location.href);
    if (!doc || !htmlFilename) {
      return event;
    }
