...
import { guichetToPLMétier } from '@/utils/guichetToPLMetier'

import {
	AvertissementActivitéNonDisponible,
	estNonDisponible,
} from './_components/ActiviteNonDisponible'
				...
				<Message>
					<Trans i18nKey="pages.assistants.choix-statut.détails-activité.aide-bnc-bic">
						<Body>
							...
							titre <Strong>independant</Strong> (avec des revenus de type BNC)
						...
						<Ul>
							...
							<Li>
								...
								medecine, la consultation ou l'écriture, vous serez probablement
		...
		(codeGuichet: GuichetEntry['code'] | undefined) => {
			...
			if (!guichet) {
				dispatch(
					enregistreLesRéponsesAuxQuestions({
						...
						'dirigeant . independant . PL . métier': O.none(),
			...
			dispatch(
				enregistreLesRéponsesAuxQuestions({
					...
					'dirigeant . independant . PL . métier': O.fromNullable(PLRMétier),
